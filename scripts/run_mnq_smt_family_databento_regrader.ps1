...
try {
    uv run --no-project --with databento python scripts\databento_mes_capability_probe.py
    if ($LASTEXITCODE -ne 0) { throw "Databento capability probe exited $LASTEXITCODE" }
    $raw = uv run --no-project --with databento --with pandas --with numpy python scripts\mnq_smt_family_databento_regrader.py --download --max-mbo-cost 4.50 --max-ohlcv-cost 0.15 --max-daily-cost 5.00 --max-plans 4
    if ($LASTEXITCODE -ne 0) { throw "regrader exited $LASTEXITCODE" }
    Write-Host $raw
    $report = $raw | ConvertFrom-Json
    uv run --no-project --with pandas --with numpy python scripts\mnq_smt_family_evidence_status.py
